...
public class MainFrame extends JFrame {
	 ...
	 * panel.
	 */
	public void render(int debugX, int debugY) {
		...
		if (debugX >=0 && debugY >= 0) {
			System.out.println("Debugging pixel " + debugX + "," + debugY);
			Color outcol = renderer.traceRay(debugX,  debugY);
			if (imagePanel.getImage() != null)
				canvas = imagePanel.getImage();
			canvas.setRGB(debugX,  debugY, outcol.getRGB());
			final int markerSize = 5;
			Graphics2D g = canvas.createGraphics();
			g.draw(new Ellipse2D.Float(debugX-markerSize/2-1, debugY-markerSize/2-1, markerSize, markerSize));
			showImage(canvas);
		} else {
